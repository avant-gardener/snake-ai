...
def display_score(display_text, display_text_2):
...
def play_game(snake_position, snake_head, apple_position, neural_n):
    crashed = False
    crashed_counter = 2
    ...
    while crashed_counter > 0:
        if neural_n:
            network_move = np.argmax(snake_network.feedforward(neural_network_input))
...
if __name__ == "__main__":
    ...
    def game(neural):
        ...
        if neural:
            final_score, training_data = play_game(snake_position_start, snake_head_start, apple_position_start, neural)
            return final_score
        else:
            final_score, training_data = play_game(snake_position_start, snake_head_start, apple_position_start, neural)
            ...
            display_score(final_text, final_text_2)
